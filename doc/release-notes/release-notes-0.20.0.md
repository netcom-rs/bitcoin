...
Lightcoin Core version 0.20.0 is now available from:

  <https://lightcoincore.org/bin/lightcoin-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Lightcoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Lightcoin Core on
unsupported systems.

From Lightcoin Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Lightcoin Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `LIGHTCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Lightcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Lightcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Lightcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Lightcoin Core. Lightcoin Core logs debug messages
...
- OpenSSL is no longer used by Lightcoin Core.  (#17265)

- BIP70 support has been fully removed from Lightcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/lightcoin/lightcoin/issues/16599),
  [PR #16702](https://github.com/lightcoin/lightcoin/pull/16702), and the
  `lightcoind help` for more information.  This option is experimental and
...
- All custom settings configured when Lightcoin Core starts are now
...
- The "Start Lightcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://lightcoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Lightcoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Lightcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/lightcoin-core/HWI) for
  signing.  Future versions of Lightcoin Core should support a GUI option
...
- Lightcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.lightcoincore.org.  (#17596)
...
- The `lightcoin-cli` utility used with the `-getinfo` parameter now
...
- The `lightcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `lightcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_lightcoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBLIGHTCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_lightcoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBLIGHTCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for lightcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate lightcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest lightcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_lightcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (lightcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone lightcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Lightcoin Core files (hebasto)
...
- #17119 Fix broken lightcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from lightcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.lightcoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to lightcoin-wallet (hel-o)
...
- #16115 On lightcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from lightcoin-cli -getinfo (malevolent)
...
[Transifex](https://www.transifex.com/lightcoin/lightcoin/).
