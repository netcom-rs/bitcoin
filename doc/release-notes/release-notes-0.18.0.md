Lightcoin Core version 0.18.0 is now available from:

  <https://lightcoincore.org/bin/lightcoin-core-0.18.0/>
...
`contrib/init/lightcoind.service`, it has been changed to use
`/var/lib/lightcoind` as the data directory instead of
`~lightcoin/.lightcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/lightcoind` will
exist has enough space (check using `df -h /var/lib/lightcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/lightcoin/lightcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://lightcoincore.org/en/contact/) the Lightcoin Core
...
The systemd init file (`contrib/init/lightcoind.service`) has been changed
to use `/var/lib/lightcoind` as the data directory instead of
`~lightcoin/.lightcoin`. This change makes Lightcoin Core more consistent with
...
`datadir` option in `/etc/lightcoin/lightcoin.conf` will have no effect.
...
- A new short [document](https://github.com/lightcoin/lightcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/lightcoin/lightcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/lightcoin/lightcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/lightcoin/lightcoin/blob/master/doc/lightcoin-conf.md)
...
- A new document introduces Lightcoin Core's BIP174 [Partially-Signed
  ...
  (PSBT)](https://github.com/lightcoin/lightcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/lightcoin/lightcoin/blob/master/doc/descriptors.md)
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/lightcoin/lightcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/lightcoin/lightcoin/blob/master/doc/descriptors.md).
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/lightcoin/lightcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/lightcoin/lightcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- A sub-project of Lightcoin Core now provides Hardware Wallet Interaction
  ...
  page](https://github.com/lightcoin-core/HWI#readme) for details.
...
- #12490 Remove deprecated wallet rpc features from `lightcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `liblightcoin_server.a` (jnewbery)
...
- #13926 [Tools] lightcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like lightcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `lightcoin-qt -version` output formatting (hebasto)
...
- #11625 Add LightcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `liblightcoin_server` (laanwj)
...
- #15154 Configure: lightcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/lightcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Lightcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `liblightcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in lightcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Lightcoin Core (Christewart)
...
- #12246 Bugfix: Only run lightcoin-tx tests when lightcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_lightcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_lightcoin` (MarcoFalke)
...
- #14186 lightcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_lightcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update lightcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/lightcoin-conf.md (hebasto)
...
- #15354 Add missing `lightcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct lightcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support lightcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/lightcoin/).
