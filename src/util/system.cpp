...
const char * const LIGHTCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/lightcoin/lightcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //lightcoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", LIGHTCOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Lightcoin
    // macOS: ~/Library/Application Support/Lightcoin
    // Unix-like: ~/.lightcoin
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", LIGHTCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Lightcoin Core copyright is not removed by accident
    if (copyright_devs.find("Lightcoin Core") == std::string::npos) {
